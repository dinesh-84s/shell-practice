...
mkdir -p $LOGS_FOLDER
echo "statred and execute the skript at with: $(date)" &>>$LOG_FILE

if [ $USERID -ne 0 ]
then
    echo -e $R "ERROR:: please run the script with using root access $N" &>>$LOG_FILE
...
then 
    ...
    echo "Mysql not installaed is going to be.. installed" &>>$LOG_FILE
    dnf install mysql -y
     VALIDATE $? "Mysql"
else 
...
if [ $? -ne 0 ]
then 
    echo "python3 not installaed is going to be.. installed" &>>$LOG_FILE
...
if [ $? -ne 0 ]
then 
    echo "nginx not installaed is going to be.. installed" &>>$LOG_FILE
    dnf install nginx -y
    VALIDATE $? "nginx"
else 
